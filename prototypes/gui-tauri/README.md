...
3. Settings: load and save the current streaming settings snapshot, including core display, bitrate, audio/video, input, warning, network, and stream-behavior preferences, plus native default bitrate calculation and frontend/native validation for numeric and select stream settings.
