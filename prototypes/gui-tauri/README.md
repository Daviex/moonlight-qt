...
The native helper validates request envelopes before dispatching commands. Malformed JSON, missing or non-integer request IDs, missing command objects, missing command names, and non-object payloads return explicit error frames instead of being coerced into request ID `0` or an empty command.
